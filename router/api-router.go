...
func SetApiRouter(router *gin.Engine) {
	...
	anonymousRequestBodyLimit := middleware.AnonymousRequestBodyLimit()
	{
		...
		apiRouter.GET("/diagnose/paypal", controller.DiagnosePayPal)  // 诊断 PayPal 配置
		...
		apiRouter.POST("/nowpayments/notify", anonymousRequestBodyLimit, controller.NowPaymentsIPN)
		...
		userRoute := apiRouter.Group("/user")
		{
			...
			// GetTopUpInfo only returns global payment config, no user-specific data.
			// No auth required so PayPal shows even if session cookie has issues.
			...
			selfRoute.Use(middleware.UserAuth())
			{
				...
				selfRoute.POST("/nowpayments/pay", middleware.CriticalRateLimit(), controller.CreateNowPaymentsPayment)
